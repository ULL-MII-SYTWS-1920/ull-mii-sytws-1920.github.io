...
Como es habitual, se supone que la callback `cb` es llamada por `asyncfun` cuando esta termina su tarea asíncrona con el primer parámetro `err` indicando el error si lo hubo y el segundo `data` con el resultado de la operación asíncrona:  `cb(err, data)`.
...
3. Deberá mostrar el tiempo que ha tomado la ejecución en nanosegundos (use [`process.hrtime.bigint`](https://nodejs.org/api/process.html#process_process_hrtime_bigint) para ello)
